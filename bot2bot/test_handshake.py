...
class FakeStamp:
    ...
    def register(self, bot_id, tier=3, attestation_grade=True, age=0):
        self.bots[bot_id] = {
            "bot_id": bot_id,
            "tier": tier,
            "attestation_grade": attestation_grade,
            "issued_at": int(time.time()) - age,
            "fingerprint_hash": f"hash-{bot_id}",
        }
    ...
    def check_denylist(self, fingerprint_hash):
        return {"listed": fingerprint_hash in self.denylist, "level": "exact" if fingerprint_hash in self.denylist else "none"}
