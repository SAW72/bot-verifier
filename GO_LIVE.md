...
- [ ] **Panel seat (Gate B)** — `script/OpsDisputePanel.s.sol` (`OpsDisputePanelSeat`). `CORE_TIMELOCK` calls `DisputePanel.setArbitrator` three times. `openDispute` reverts `panel not seated` until `arbitratorCount >= 3`. Prefer this before the escrow broadcast. Simulate has no `--broadcast`. See `script/DEPLOY_ESCROW_BASE_SEPOLIA.md`.
- [ ] **(2) Escrow** — `script/DeployBotAttestationEscrow.s.sol`. Env: `PRIVATE_KEY`, `DENYLIST`, `VAULT`, `DISPUTE_PANEL`, `CORE_TIMELOCK`, `BASE_SEPOLIA_RPC_URL` (addresses required, non-zero; timelock ≠ deployer). Live addresses are pre-filled in `script/DEPLOY_ESCROW_BASE_SEPOLIA.md`. Simulate without `--broadcast`. Script `transferOwnership(CORE_TIMELOCK)`; timelock must `acceptOwnership`. Do not `createEscrow` before that accept. Then paste the real address into `deployments/base-sepolia.json` and `contracts/README.md`. Do not invent it.
