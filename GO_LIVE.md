...
- [ ] **(1a) Denylist + Vault redeploy** — `script/DeployDenylist.s.sol` replaces the live pre–PR #10 Denylist with tip bytecode. Env: `PRIVATE_KEY`, `CORE_TIMELOCK` (≠ deployer). Simulate: `forge script script/DeployDenylist.s.sol:DeployDenylist --rpc-url $BASE_SEPOLIA_RPC_URL -vvvv`. Spencer broadcasts. Then `acceptOwnership` on both new contracts and migrate active listings. See `script/DEPLOY_DENYLIST.md`. Do not re-run `Deploy.s.sol` for this cutover. Spencer fills `deployments/base-sepolia.json` with the real addresses after broadcast.
